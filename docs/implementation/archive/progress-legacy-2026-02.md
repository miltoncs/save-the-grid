...
## 2026-02-23 (Docs terminology: powerplants)
- Standardized documentation terminology from `plant` / `plants` to `powerplant` / `powerplants`.
- Updated core design docs, mode docs, tutorial docs, UX docs, and implementation terminology snippets to remove bare `plant(s)` wording.
- Updated connection examples and tool-label references to the same vocabulary.
...
## 2026-02-24 (Export metadata: commit hash + full parameter snapshot)
- Updated `tools/terrain/interactive/app.js` export pipeline so every exported metadata file includes:
  - `commit_hash` (resolved from local git repository via `/.git/HEAD`, `refs`, and `packed-refs` fallback).
  - `exported_at_utc` timestamp.
  - `generation_parameters` object with three sections:
    - `terrain`: algorithm, seeds, smoothness, continent scale, sea level, snowcaps, requested/generated rivers, river width.
    - `resources`: selected type, zone strength, vertex snap, draft count, zone count, and per-zone summaries.
    - `visual_effects`: shoreline/river tint toggles and all shadow controls.
- Added async commit resolution before export (`exportBundle` waits for commit hash resolution first).
- Added non-blocking commit prefetch on startup (`void ensureCommitHashResolved()`).

### Validation
- `node --check tools/terrain/interactive/app.js`
- Develop-web-game Playwright loop run completed.
- End-to-end export validation with Playwright downloads:
  - Export created both files (`.png` and `.metadata.json`).
  - Metadata contains non-empty `commit_hash` (`4cfe337c8c8ae2a8e17c0be106b9fa7aa3c8b5ae` in test run).
  - Metadata contains `generation_parameters.terrain`, `.resources`, and `.visual_effects` with expected values.

## 2026-02-25 (Tutorial map source switch)
- Updated terrain map profile resolution so tutorial mode always uses assets under `assets/maps/tutorial`.
- Added tutorial map constants in `src/game/core.js`:
  - `TUTORIAL_TERRAIN_MAP_ID = "tutorial-core"`
  - `TUTORIAL_TERRAIN_MAP_IMAGE_URL = "/assets/maps/tutorial/terrain-map-4143667724.png"`
  - `TUTORIAL_TERRAIN_MAP_METADATA_URL = "/assets/maps/tutorial/terrain-map-4143667724.metadata.json"`
- `resolveTerrainMapProfile(config)` now returns this tutorial profile when `config.mode === "tutorial"`.
- Validation:
  - Syntax checks: `node --check src/game/core.js`, `node --check src/game.js`, `node --check src/main.js` passed.
  - Runtime check via Playwright confirmed tutorial state now reports:
    - `terrainMap.id: tutorial-core`
    - `terrainMap.image: /assets/maps/tutorial/terrain-map-4143667724.png`
    - `terrainMap.metadata: /assets/maps/tutorial/terrain-map-4143667724.metadata.json`
    - map loaded true.
  - Screenshot: `output/web-game/tutorial-map-source/shot.png`.
