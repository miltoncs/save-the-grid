...
const EXPORT_CONTENT_VERSION = 1;
const EXPORT_COORDINATE_SYSTEM = "origin_top_left_y_down";
const EXPORT_IMAGE_BASE_PATH = "/assets/maps/terrain";
const EXPORT_METADATA_BASE_PATH = "/data/maps/terrain";
...
function buildExportDefaultTown(width, height) {
  return {
    id: "capital",
    name: "Johnsonville",
    ...
    strategicValue: "Placeholder town generated by Terrain Lab export",
  };
}
...
function buildExportDocuments(mapId, displayName, output) {
  ...
  const metadataUrl = `${EXPORT_METADATA_BASE_PATH}/${metadataFileName}`;
  ...
  const mapDocument = {
    contentVersion: EXPORT_CONTENT_VERSION,
    mapId,
    displayName,
    world: {
      width: output.width,
      height: output.height,
      coordinateSystem: EXPORT_COORDINATE_SYSTEM,
    },
    terrainMap: {
      imageUrl,
      metadataUrl,
    },
    towns: [buildExportDefaultTown(output.width, output.height)],
    links: [],
    resourceZones,
  };
...
function getExportContext() {
  ...
  const { mapDocument, metadata, fileNames } = buildExportDocuments(
    mapId,
    displayName,
    state.currentOutput
  );
...
async function exportMapJsonOnly() {
  const { mapDocument, fileNames } = getExportContext();
  triggerDownload(
    new Blob([`${JSON.stringify(mapDocument, null, 2)}\n`], { type: "application/json" }),
    fileNames.map
  );
  setExportStatus(`Exported ${fileNames.map}`);
}

async function exportMetadataJsonOnly() {
  const { metadata, fileNames } = getExportContext();
  triggerDownload(
    new Blob([`${JSON.stringify(metadata, null, 2)}\n`], { type: "application/json" }),
    fileNames.metadata
  );
  setExportStatus(`Exported ${fileNames.metadata}`);
}

async function exportMapPngOnly() {
  const { fileNames } = getExportContext();
  await exportCurrentMapPng(fileNames.image);
  setExportStatus(`Exported ${fileNames.image}`);
}
...
function updateStats(stats) {
  ...
  if (state.editorMode === "visual-effects") {
    ...
    statsValue.textContent = `${baseStats} | ${shorelineState} | ${riverState}`;
    return;
  }
...
if (exportMapJsonBtn) {
  exportMapJsonBtn.addEventListener("click", async () => {
    try {
      await exportMapJsonOnly();
    } catch (error) {
...
if (exportMetadataJsonBtn) {
  exportMetadataJsonBtn.addEventListener("click", async () => {
    try {
      await exportMetadataJsonOnly();
    } catch (error) {
      setExportStatus(`Export failed: ${error.message}`, true);
    }
  });
